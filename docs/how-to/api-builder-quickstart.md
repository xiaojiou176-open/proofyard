...
the shortest truthful path into Webaudit's contract layer.

Use this page when you are:

- wiring Webaudit into another script or service
...
agent should stay in charge of reasoning while Webaudit supplies browser
...
```ts
// Example: a repo-local script launched from the Webaudit workspace root.
import {
...
- read [Webaudit for Coding Agents and Agent Ecosystems](webaudit-for-coding-agents.md) if
  your search intent started from a coding-agent shell
- read [Webaudit for AI Agents](webaudit-for-ai-agents.md) if you want the
