# Webaudit for AI Agents

Webaudit matters to AI-agent builders for one practical reason:
...
This page is intentionally narrow. It does **not** claim that Webaudit is a
generic AI agent platform. It explains why an AI-agent team would still care
about this repo.

It is also the right page when your team is evaluating Webaudit as the
...
- why would I use Webaudit instead of wiring a raw browser bot into my agent
  loop?
...
[Webaudit for Coding Agents and Agent Ecosystems](webaudit-for-coding-agents.md)
after this page.
...
AI agents do not get a magic autonomy shell from Webaudit.
...
4. **A governed MCP road**
   - let an external AI client consume tools without turning Webaudit into a
     hidden second backend

## If you are using coding-agent or agent-stack workflows

Webaudit does **not** claim a dedicated first-class Codex integration or a
...
- a coding agent plans and reasons outside Webaudit
- Webaudit handles browser execution, retained evidence, and recovery
...
| --- | --- |
| browser automation for Codex | Webaudit can be the browser-evidence layer behind a Codex-style agent loop |
| browser automation for Claude Code | Webaudit can be the browser execution and evidence layer behind Claude Code style workflows |
| browser automation for OpenHands | Webaudit can sit behind an OpenHands-style runtime as the browser-evidence and recovery subsystem |
| browser automation for OpenCode | Webaudit can provide a governed browser tool layer beneath an OpenCode-style shell |
| browser automation for OpenClaw | Webaudit can act as the browser workflow backend behind an OpenClaw-style gateway or tool router |
| MCP server for coding agents | Webaudit exposes a governed MCP surface on top of the same backend and artifacts |
| browser automation API for AI agents | Webaudit exposes a contract-first API for direct integration |

This is a strong fit for **Codex-, Claude Code-, OpenHands-, OpenCode-, and
OpenClaw-style agent stacks** without pretending that Webaudit is the whole
coding agent.
...
- yes, Webaudit can fit those workflows well
- no, Webaudit is not claiming an official Codex integration
- no, Webaudit is not claiming an official Claude Code integration
- no, Webaudit is not claiming official OpenHands, OpenCode, or OpenClaw integrations
- yes, Webaudit can still be the browser-execution, retained-evidence, and
...
- **Webaudit as the browser-evidence and recovery substrate**
...
Webaudit is not trying to beat generic browser agents at open-ended roaming.
...
- Webaudit stays the browser, evidence, and recovery layer
...
Webaudit fits best as the browser execution and evidence layer inside a
larger system.

Think of the stack like this:

| Layer | What stays outside Webaudit | What Webaudit contributes |
| --- | --- | --- |
...
This is why Webaudit can matter to AI agents without claiming to be the whole
agent platform.
...
6. [Webaudit vs Generic Browser Agents](../compare/webaudit-vs-generic-browser-agents.md)
