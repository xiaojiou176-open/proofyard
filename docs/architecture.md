...
Webaudit is a monorepo for evidence-first browser automation.
...
Webaudit has three related but distinct execution concepts. Treating them as
...
That means Webaudit now exposes a stronger recovery assistant, but it still does not ship an autonomous self-heal loop.
