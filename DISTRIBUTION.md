...
| ClawHub skill page | yes | yes | yes | The ProofTrail MCP skill page can be read back live at `https://clawhub.ai/skills/proofyard-mcp`. This is a live public discovery page for the skill packet, not proof of a hosted MCP endpoint or generic skill-registry publication. |
| Repo-owned ProofTrail MCP install skill | yes | yes | no | `skills/proofyard-mcp/` is materialized in the repo, and its current external receipts are now split across ClawHub live, Goose review-pending, agent-skill review-pending, awesome-opencode review-pending, and OpenHands closed-not-accepted. |
