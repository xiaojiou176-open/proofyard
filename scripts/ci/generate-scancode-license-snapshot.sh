...
tmp_root="${TMPDIR:-/tmp}"
mkdir -p "${tmp_root%/}"
snapshot_tmp="$(mktemp "${tmp_root%/}/webaudit-scancode-snapshot.XXXXXX")"
rm -f "$snapshot_tmp"
snapshot_tmp="${snapshot_tmp}.json"
work_dir="$(mktemp -d "${tmp_root%/}/webaudit-scancode-targets.XXXXXX")"
...
payload = {
    "headers": [
        {
            "tool_name": "webaudit-manifest-fallback",
            "tool_version": "1",
