...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/webaudit-space-clean-reclaim-XXXXXX")"
...
assert candidates["repo-pnpm-store"]["path"] == str(home_root / ".cache/pnpm/webaudit/store")
...
assert not (home_root / ".cache/pnpm/webaudit").exists()
