...
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/proofyard-space-report-XXXXXX")"
...
assert any(layer["id"] == "pnpm-proofyard-store" for layer in payload["repo_exclusive_external_layers"])
...
assert home_root.joinpath(".cache/pnpm/proofyard").resolve() in external_paths
PY
