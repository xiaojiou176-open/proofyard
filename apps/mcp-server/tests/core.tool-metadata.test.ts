...
nodeTest("package metadata, runtime identity, and skill manifest stay aligned", () => {
  ...
  const skillManifest = yaml.parse(
    readFileSync(resolve(repoRoot, "skills/proofyard-mcp/manifest.yaml"), "utf8")
  )

  assert.equal(packageJson.name, "@proofyard/mcp-server")
  assert.equal(packageJson.version, "0.1.1")
  assert.equal(packageJson.license, "MIT")
  assert.equal(packageJson.publishConfig?.access, "public")
  assert.equal(packageJson.bin?.["proofyard-mcp"], "./dist/server.cjs")
  assert.ok(Array.isArray(packageJson.files) && packageJson.files.includes("dist"))
  assert.match(coreSource, /name:\s*"@proofyard\/mcp-server"/)
  assert.match(coreSource, /version:\s*"0\.1\.1"/)
  assert.equal(skillManifest.name, "proofyard-mcp")
