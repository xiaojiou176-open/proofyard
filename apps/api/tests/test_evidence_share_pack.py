...
client = TestClient(
    ...
    headers={
        ...
        "x-automation-client-id": "pytest-evidence-share",
    },
)
...
def _write_manifest(run_dir: Path, run_id: str) -> None:
    ...
    manifest = {
        ...
        "gateResults": {"status": "failed", "checks": []},
...
def test_evidence_share_pack_api_returns_markdown_and_issue_digest(tmp_path: Path) -> None:
    runs_root = tmp_path / "artifacts" / "runs"
    _write_manifest(runs_root / "run-a", "run-a")

    response = client.get("/api/evidence-runs/run-a/share-pack")
    assert response.status_code == 200
    share_pack = response.json()["share_pack"]
    assert share_pack["run_id"] == "run-a"
    assert "Evidence Share Pack" in share_pack["markdown_summary"]
    assert "Failure Digest" in share_pack["issue_ready_snippet"]
    assert share_pack["json_bundle"]["retention_state"] == "retained"
