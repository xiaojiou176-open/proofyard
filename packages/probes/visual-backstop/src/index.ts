...
const BACKSTOP_NOT_AVAILABLE = "gate.visual_engine.blocked.backstop_not_available"

function hasBackstopRuntime(): boolean {
  const require = createRequire(import.meta.url)
  try {
    require.resolve("backstopjs")
    return true
  } catch {
    return false
  }
...
export async function runVisualWithBackstopBridge(
  baseDir: string,
  config: VisualConfig,
  fallback: (baseDir: string, config: VisualConfig) => Promise<VisualResult>
): Promise<VisualResult> {
  if (!hasBackstopRuntime()) {
    const blocked: VisualResult = {
      engine: "backstop-bridge",
      engineUsed: "backstop",
      executionStatus: "blocked",
      blockedReasonCode: BACKSTOP_NOT_AVAILABLE,
      blockedDetail: "backstopjs package not installed",
      url: config.baseUrl,
      mode: config.mode,
